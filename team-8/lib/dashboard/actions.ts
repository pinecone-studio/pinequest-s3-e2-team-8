...
import { loadStudentExamAccessContext } from "@/lib/student-exam-access";
...
const getCachedStudentLearningDashboardSummary = unstable_cache(
  async (studentId: string) => getStudentLearningDashboardSummary(studentId),
  ["student-learning-dashboard-summary"],
  { revalidate: 60 },
);
...
export async function getStudentStats() {
  ...
  const latestSessionMap = new Map<
    string,
    { status: string | null; attemptNumber: number; startedAt: string | null }
  >();
  ...
  const decoratedExams: StudentUpcomingExam[] = Array.from(
    accessContext.accessMap.values(),
  ).map((exam) => ({
    id: exam.id,
    title: exam.title,
    start_time: exam.start_time,
    ...
    session_status: exam.mySessionStatus,
    lifecycle_status: exam.myLifecycleStatus,
    lifecycle_label: exam.myLifecycleLabel,
  }));
  ...
  try {
    learningSummary = await getCachedStudentLearningDashboardSummary(user.id);
  } catch (error) {
